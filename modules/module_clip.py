...
def _default_ast_path() -> str:
    """Resolve AST checkpoint path: env var ``AST_PATH`` first, then repo default."""
    p = os.environ.get("AST_PATH")
    if p:
        return p
    return os.path.join(_REPO_ROOT, "pretrained", "AST", "audioset_10_10.pth")
...
#             print('ImageNet pretraining: {:s}, AudioSet pretraining: {:s}'.format(str(imagenet_pretrain),str(audioset_pretrain)))
        ...
        if audioset_pretrain == False:
            ...
            audio_pretrained_path = _default_ast_path()
...
class CLIP(nn.Module):
    ...
    def get_config(pretrained_clip_name="ViT-B/32"):
        ...
        audio_pretrained_path = _default_ast_path()
        sd = torch.load(audio_pretrained_path, map_location='cpu')
        for n, p in sd.items():
            new_name = 'audio.' + n.replace('module.', '')
            state_dict[new_name] = p
