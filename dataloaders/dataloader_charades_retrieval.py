...
def _teacher_dir() -> str:
    """ImageBind teacher feature root for Charades.

    Order: ``$SAVE_CHARADES_TEACHER_DIR`` > ``<repo>/data/charades/FeatureData/ImageBind``.
    """
    p = os.environ.get("SAVE_CHARADES_TEACHER_DIR")
    if p:
        return p
    return os.path.join(_REPO_ROOT, "data", "charades", "FeatureData", "ImageBind")
